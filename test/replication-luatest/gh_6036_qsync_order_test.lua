...
g.before_all(function(cg)
    ...
    cg.r1 = cg.cluster:build_and_add_server({ alias = 'r1', box_cfg = cg.box_cfg })
    cg.r2 = cg.cluster:build_and_add_server({ alias = 'r2', box_cfg = cg.box_cfg })
    cg.cluster:start()
end)
...
g.before_test("test_qsync_order", function(cg)
    ...
    cg.r3 = cg.cluster:build_and_add_server({ alias = 'r3', box_cfg = cg.box_cfg })
    cg.r3:start()
...
g.test_qsync_order = function(cg)
    ...
    t.assert_equals(cg.r1:exec(function() return box.space.test:select() end), {{1}})
    t.assert_equals(cg.r2:exec(function() return box.space.test:select() end), {{1}})
    t.assert_equals(cg.r3:exec(function() return box.space.test:select() end), {{1}})
    ...
    t.assert_equals(cg.r3:exec(function() return box.space.test:select() end), {{1},{2}})
end
...
g.test_promote_order = function(cg)
    ...
    t.assert_equals(cg.r2:exec(function() return box.space.test:select() end), {{1},{2}})
end
